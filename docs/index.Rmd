...
opioid_per_head_chart <- opioid_per_head_long %>% 
  ...
  labs(title = "Figure 2: Prescription of Tramadol/Codeine/Dihydrocodeine Per Person in Each Health Board",
       x = "Health Board",
       y = "Quantity Per Head",
       caption = "Note: 'Tramadol' = 50/100/150/200 (mg). 'Codeine' = 15/30/60 (mg). 'Dihydrocodeine' = 30/60 (mg)") +
...
msk_hospital_referrals <- full_join(health_boards, msk_hospital_referrals_data, join_by(hb == hbt)) %>% 
  ...
  filter(referral_source == "All Referral Sources",
         ...
         !hb == "S92000003") %>%
  group_by(hb, hb_name) %>% 
  summarise(total_referrals_per100k= sum(referrals_per_one_hundred_thousand_population))
...
tramadol_msk_plot <- tramadol_msk_data %>% 
  ...
  labs(title= "Figure 3: Comparison Between Musculoskeletal Pain Referrals and Tramadol Sales", 
       ...
       x="Health Board", y="Referrals for Musculoskeletal Pain Per 100,000 Population") + 
theme(plot.caption = element_text(hjust=0))
...
```{r opioid stats in 2015 v 2020 - loading prescription data}
...
```{r opioid stats in 2015 v 2020 - wrangling prescription code}
...
```{r opioid stats in 2015 v 2020 - opioid dependency code}
# required data
...
opioid_change
```
As seen in Figure 4, codeine and buprenorphine prescriptions increased by 2.7% and 89.8% respectively, while tramadol and dihydrocodeine prescriptions decreased  by 8.0% and 15.2% respectively. Overall, opioid count decreased from a count of 7503 in 2015 to a count of 7200 in 2020, marking a decrease of approximately 4%. This decrease could have been a result of the pandemic in that there may have been reduced opioid prescriptions overall or even difficulty in terms of accessing prescriptions. 

Despite the overall decrease in opioid count, interestingly during the same time period, both opioid related hospitalisations and deaths had a significantly large increase at 43.5% and 109% respectively. This suggests that there is unlikely to be a relationship between the number of opioid prescribed and
opioid related hospitalisations and deaths or that it is difficult to ascertain due to the effect of the pandemic. It would be interesting to compare changes after 2020 if more recent opioid dependency data was made available. 
